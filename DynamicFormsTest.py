...
def create_Anasthesia_Technician():
    ...
    MedicalRecordsTechnician = st.number_input("Medical Records Technician", min_value=0, max_value=40, step=1)
    CallCenterRepresentative = st.number_input("Call Center Representative", min_value=0, max_value=40, step=1)
    PatientServicesAssistant = st.number_input("Patient Services Assistant", min_value=0, max_value=40, step=1)
    ...
    if st.button("Submit"):
        data = {
            ...
            "Employee ID": EmpID,
            "Job Title": JobTitle,
            "Direct Experience": DirectExp,
            "Highly Related Experience": MedicalAssistant + PatientCareTechnician + SurgicalTechnologist + NurseAssistant + Phlebotomist + PharmacyTechnician,
            "Related Experience": MedicalRecordsTechnician + CallCenterRepresentative + PatientServicesAssistant
        }
        df = pd.DataFrame([data])
        df.to_csv('Anasthesia_Technician.csv', mode='a', index=False, header=False)
        st.success("Data saved to Anasthesia_Technician.csv")
...
def create_Athletic_Trainer():
    ...
    if st.button("Submit"):
        data = {
            ...
            "Job Title": JobTitle,
            "Direct Experience": DirectExp,
            "Highly Related Experience": SpecialtyExperienceCertification + AdditionalHRE
        }
        df = pd.DataFrame([data])
        df.to_csv('Athletic_Trainer.csv', mode='a', index=False, header=False)
        st.success("Data saved to Athletic_Trainer.csv")
...
def create_Art_Therapist():
    ...
    if st.button("Submit"):
        data = {
            ...
            "Employee ID": EmpID,
            "Job Title": JobTitle,
            "Direct Experience": ArtTherapist + ATFellowship,
            "Highly Related Experience": ActivityRecreationTherapy + ArtTeacher + Counselor + AdjunctProfessor,
            "Related Experience": NonArtFormalTeaching + ProgramExperience + DirectPatientCare + VisualArtsMultimedia + ResearchPosition,
            "Education Credit": EdCredit,
            "Certification Credit": ATRBS + CTRS + MTBC + LLPC + LLP + LMSW + 0.5 * (InfantMassage + FLE + CPXP)
        }
        df = pd.DataFrame([data])
        df.to_csv('Art_Therapist.csv', mode='a', index=False, header=False)
        st.success("Data saved to Art_Therapist.csv")
...
def create_Audiologist():
    ...
    st.header("Certification Credit, enter 1 for each ceritification, max 1 point")
    AmericanSign = st.number_input("American Sign Language Certification (0.5 points)", min_value=0, max_value=1, step=1)
    Deaf = st.number_input("Deaf/Hard of hearing Teaching Certification (0.5 points)", min_value=0, max_value=1, step=1)
    ...
    if st.button("Submit"):
        data = {
            ...
            "Job Title": JobTitle,
            "Direct Experience": DirectExp,
            "Highly Related Experience": HRExpCalc,
            "Certification Credit": AmericanSign + Deaf
        }
        df = pd.DataFrame([data])
        df.to_csv('Audiologist.csv', mode='a', index=False, header=False)
        st.success("Data saved to Audiologist.csv")
